...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        with connection.cursor() as cursor:
            ...
            cursor.execute("ALTER TABLE datacenters MODIFY countrydomain VARCHAR(2)")

            # Tiny int is enough for boolean fields
            cursor.execute("ALTER TABLE datacenters MODIFY residualheat TINYINT(1)")

            cursor.execute("ALTER TABLE datacenters MODIFY virtual TINYINT(1)")

            cursor.execute("ALTER TABLE datacenters MODIFY dc12v TINYINT(1)")
            ...
            try:
                ...
                self.stdout.write(
                    "You ran the migration twice, skipping adding password column"
                )
            ...
            try:
                cursor.execute(
                    "CREATE UNIQUE INDEX username_unique_index ON fos_user (username);"
                )
            except Exception:

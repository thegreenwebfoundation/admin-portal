/**
 * This is concatenated set of the libraries on the admin hosting provider change form
 * that when loaded in a production environment, fail to load in the correct order
 * resulting in errors because jQuery or the `$` variable is undefined at the time of
 * javascript code being loaded. the text below contains the following scripts:
 * 
 * Dependencies used by django
 ...
 *  
 * 
 * It is only expected to be loaded on the hostingprovider change_form
 * as an override to the default `{{ media}}` bundle
 */
